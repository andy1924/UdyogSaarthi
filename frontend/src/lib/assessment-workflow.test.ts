...
const complete: WorkflowRequirements = { userCoords: { lat: 18.5, lon: 73.8 }, locationText: 'Pune, Maharashtra', selectedEnterprise: 'food', feasibilityResult: feasibility, schemeResult: scheme, applicantName: 'Asha Patil', fundingPreference: 'scheme_linked_loan' };
...
describe('strict assessment workflow', () => {
  ...
  it('validates each mandatory stage before progression', () => {
    ...
    expect(getAdvanceError(5, { ...complete, fundingPreference: '' })).toMatch(/fund/i);
