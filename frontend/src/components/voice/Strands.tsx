/**
 * Strands - the ReactBits animation (reactbits.dev/animations/strands), adapted
 * for the voice orb.
 *
 * One local change, marked in place below: `level` hands the component the live
 * voice level. The orb has to swell with whoever is talking, and an audio level
 * must never travel through React state, so this is a getter the render loop
 * reads once a frame.
 *
 * Everything else is the published component.
 */
...
export interface StrandsProps {
  ...
  dispersion?: number;
  glassSize?: number;
  /** Live voice level, 0-1, read once per frame. Local addition. */
  level?: () => number;
  className?: string;
  style?: CSSProperties;
}
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const frame = (t: number) => {
      ...
      // Local addition: the voice level, 0-1. It lifts the wave and thickens the
      // glow, so the strands swell with whoever is speaking.
      const voice = Math.min(Math.max(current.level(), 0), 1);
      ...
      program.uniforms.uAmplitude.value = current.amplitude * (1 + voice * 1.5);
      ...
      program.uniforms.uIntensity.value = Math.min(current.intensity + voice * 0.3, 1);
