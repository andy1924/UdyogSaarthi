...
def nearby(
    ...
    for i in range(n):
        d = 400 + (
            hashlib.md5(f"{lat}:{i}".encode()).hexdigest()[0:2].__hash__() % (radius_m - 400)
        )
        ...
        profiles.append(
            NearbyProfile(
                id=f"p{i + 1}",
                name=NAMES[(h + i) % len(NAMES)],
                category=(category or ["dairy", "retail", "electronics"][i % 3]),
                distance_m=d,
                lat=lat + (i * 0.004),
                lon=lon + (i * 0.004),
            )
    ...
    sql = (
        f"SELECT * FROM profiles WHERE ST_DWithin("
        f"geom, ST_SetSRID(ST_MakePoint({lon},{lat}),4326)::geography, "
        f"{radius_m})"
    )
    if category:
        sql += f" AND category='{category}'"
    sql += (
        " ORDER BY ST_Distance(geom, ST_SetSRID(ST_MakePoint(lon,lat),4326)::geography) LIMIT 20;"
    )
    return DirectoryOut(
