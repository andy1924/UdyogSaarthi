...
app = FastAPI(title="UdyogSaarthi API", version="0.1.0", description="Deterministic scheme math + KYN feasibility + DPR + compliance/directory (mocked LGD/OSM, auditable rules v2024-11)")
...
async def healthcheck() -> dict[str, str]:
    ...
    async def _redis():
        try:
            def _ping():
                c = Redis.from_url(settings.redis_url, decode_responses=True, socket_connect_timeout=0.6, socket_timeout=0.6)
                c.ping()
                c.close()
    ...
    return {"status": "ok" if db_ok and redis_ok else "degraded", "database": "up" if db_ok else "down", "redis": "up" if redis_ok else "down"}
