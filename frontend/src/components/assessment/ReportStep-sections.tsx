...
type Status = AssessmentState['dprStatus'];

export function ReportHeader({ dprStatus, ready, statusLabel }: {
  dprStatus: Status; ready: boolean; statusLabel: string;
}) {
  return (
...
export function ReportConfirmSection({ reviewConfirmed, setReviewConfirmed, dprStatus, ready, handleDprDownload, handleShareWhatsApp }: Pick<AssessmentState,
  'reviewConfirmed' | 'setReviewConfirmed' | 'dprStatus' | 'handleDprDownload' | 'handleShareWhatsApp'
> & { ready: boolean }) {
  return (
    <section aria-labelledby="confirm-title" className="rounded-2xl border-2 border-primary/20 bg-secondary-container/30 p-5 sm:p-6">
