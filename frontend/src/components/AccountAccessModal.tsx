...
export default function AccountAccessModal({ open, onClose, onSuccess }: Props) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError(null);
    try {
      if (mode === 'register') await api.registerApplicant({ email, password, fullName });
  ...
  return (
    <div className="fixed inset-0 z-[70] grid place-items-center bg-black/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="account-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div className="relative w-full max-w-md rounded-4xl bg-white p-6 shadow-2xl sm:p-8">
        <button type="button" onClick={onClose} aria-label="Close" className="absolute right-4 top-4 grid h-11 w-11 place-items-center rounded-full bg-olive-50"><X size={20} /></button>
        ...
        <button type="button" onClick={() => { setMode(mode === 'signin' ? 'register' : 'signin'); setError(null); }} className="mt-4 w-full text-sm font-semibold text-olive-800 underline underline-offset-4">
